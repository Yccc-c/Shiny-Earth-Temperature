...
This page is built by [**flexdashboard**](https://pkgs.rstudio.com/flexdashboard/). It also use three icons on [**Font awesome**](https://fontawesome.com/)

After selecting, it will take some time to load the plots and tables, please wait patiently and don't do any other operation such as zooming the window, otherwise the server may return errors. If it occurs, please contact the author as soon as possible: **cheny_ycc@qq.com**

The shiny is divided into 3 sub-pages. Click the 3 buttons at top to see more information.

The source code is at the top-right corner. If you like it, please give me a "shiny" star. :)
...
**Average Temperature:** It use data in `*Country.csv`. So in some countries, the **Average Temperature** may be lower or hotter than any city in the country because of unbalanced distribution, such as Canada.
...
renderPlot({
  ...
  world_map %>% 
    ...
    ggplot(aes(x = long, y = lat, group = group)) +
      geom_polygon(aes(fill=AverageTemperature), colour = "white", size = 0.3) +
      geom_point(aes(x = as.double(city_current[1,7]), 
